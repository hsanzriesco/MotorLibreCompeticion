    ...
    try {
        ...
        if (method === "DELETE") {
            ...
            try {
                ...
                // 🛑 LIMPIEZA PROFUNDA (Deep Clean) - CORRECCIÓN USANDO id_club
                // Si 'id_club' no es la columna correcta, el error handler te lo dirá con el código 42703.

                // 1. Borrar inscripciones de eventos del club
                await client.query('DELETE FROM public.inscripciones WHERE event_id IN (SELECT id FROM public.events WHERE id_club = $1)', [clubId]);

                // 2. Borrar eventos
                await client.query('DELETE FROM public.events WHERE id_club = $1', [clubId]);

                // 3. Borrar posts, noticias, o publicaciones del club
                await client.query('DELETE FROM public.posts WHERE id_club = $1', [clubId]);
                await client.query('DELETE FROM public.noticias WHERE id_club = $1', [clubId]);
                await client.query('DELETE FROM public.comentarios WHERE id_club = $1', [clubId]);
        ...
        if (error.code === '42P01') {
            ...
            return res.status(500).json({ success: false, message: `Error de la base de datos: La tabla "${tableName}" no fue encontrada. (POSIBLE SOLUCIÓN: ELIMINAR LA LÍNEA DE CÓDIGO QUE REFERENCIA A ESTA TABLA)` });
        }

        // 42703: Columna no existe.
        if (error.code === '42703') {
            const columnName = error.message.match(/column "(.+?)" does not exist/)?.[1] || 'una columna desconocida';
            return res.status(500).json({ success: false, message: `Error de la base de datos: La columna "${columnName}" no fue encontrada. (SOLUCIÓN: CAMBIAR EL NOMBRE DE LA COLUMNA EN EL CÓDIGO POR EL NOMBRE REAL DE TU BASE DE DATOS)` });
        }

        // 23xxx: Violación de integridad (Foreign Key).
